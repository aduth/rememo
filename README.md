...
To simplify testing of memoized selectors, the function returned by `createSelector` includes the [moize](https://github.com/planttheidea/moize) memoized function instance on its `memoizedSelector` property. To clear the cache before each test, call `clear` on this property:
